...
const CARD = "bg-white rounded-2xl border border-[#1E2A33]/[0.06] shadow-[0_2px_14px_rgba(20,26,33,0.07)]";
...
const DEFAULT_SLOTS = [
  { id: "morningSubway", label: "Morning Transit", iconName: "Train", color: "#E29A3E" },
  { id: "morning", label: "Morning", iconName: "Sunrise", color: "#4C8FBD" },
  { id: "daytime", label: "Daytime", iconName: "Sun", color: "#2F9E8F" },
  { id: "eveningSubway", label: "Evening Transit", iconName: "Train", color: "#8B6FB3" },
  { id: "evening", label: "Evening", iconName: "Moon", color: "#C1595B" },
];

const DEFAULT_CATEGORIES = [
  { id: "notes", label: "Lesson notes", iconName: "NotebookPen", color: "#4C8FBD" },
  { id: "gatherQuestions", label: "Gather questions", iconName: "MessageCircleQuestion", color: "#E29A3E" },
  { id: "gatherTasks", label: "Gather tasks", iconName: "ListChecks", color: "#2F9E8F" },
  { id: "qa", label: "Q&A", iconName: "HelpCircle", color: "#8B6FB3" },
  { id: "solvingTasks", label: "Solving tasks", iconName: "Calculator", color: "#C1595B" },
];
...
function normalizeData(parsed) {
  ...
  if (Array.isArray(parsed.projects) && parsed.projects.length) {
    ...
    const activeProjectId = projects.some((p) => p.id === parsed.activeProjectId) ? parsed.activeProjectId : projects[0].id;
...
const goalForDate = (settings, date) => Number(settings?.dailyGoals?.[date.getDay()]) || 0;
...
const getById = (list, id, fallbackLabel) =>
  list.find((i) => i.id === id) || { id, label: fallbackLabel || id, color: "#9AA3AC", iconName: "HelpCircle" };
...
const segBtnStyle = (active) => (active ? { backgroundColor: ACCENT } : undefined);
    ...
    <div className="flex flex-wrap justify-center gap-1.5 mt-3">
      {items.map((it) => {
        ...
        return (
          ...
          >
            <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: isHidden ? "#1E2A3355" : it.color }} />
            {it.label}
          </button>
        );
      })}
...
const SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im5ncnFmdmR5eXBsY3NvbHlrYWFxIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODUwMjI0NDgsImV4cCI6MjEwMDU5ODQ0OH0.xxnqMZ91vErHp6s6OE9dY1PH1nIfHVBLhrOUexrFTsY"; // your anon/public API key
...
const CLOUD_ENABLED = /^https:\/\/[a-z0-9-]+\.supabase\.co$/i.test(NORMALIZED_SUPABASE_URL) && SUPABASE_ANON_KEY.trim().length > 20;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const sb = mod.createClient(NORMALIZED_SUPABASE_URL, SUPABASE_ANON_KEY.trim());
        setClient(sb);
    ...
    <div className="min-h-screen bg-[#F4F5F7] text-[#1E2A33] flex items-center justify-center p-4">
      <div className={`${CARD} w-full max-w-sm p-6`}>
        <div className="flex items-center gap-2 mb-1">
          ...
          <h1 className="font-sans font-extrabold uppercase tracking-tight text-lg">Study Manifest</h1>
        </div>
        ...
        {error && (
          <p className="text-[11px] font-mono text-[#C1595B] mb-3">
            Cloud sync failed to load ({String(error.message || error)}). Check your Supabase setup.
          </p>
        )}
        ...
        <form onSubmit={submit} className="space-y-3">
          ...
          {msg && <p className="text-[11px] font-mono text-[#1E2A33]/70">{msg}</p>}
...
export default function StudyTrackerApp() {
  const { client: cloudClient, session, ready: authReady, loadError, cloudEnabled } = useCloudAuth();
    ...
    async (next) => {
      ...
      try {
        if (canUseCloud) {
          await cloudClient
            .from("study_data")
            .upsert({ user_id: session.user.id, data: next, updated_at: new Date().toISOString() });
        } else {
    ...
    (patch) => {
      persist({
        ...
        projects: data.projects.map((p) =>
          p.id === project.id
            ? { ...p, ...patch, settings: patch.settings ? { ...p.settings, ...patch.settings } : p.settings }
            : p
        ),
      });
    },
  ...
  const updateDay = (key, patch) => {
    ...
    updateProject({ days: { ...project.days, [key]: { ...existing, ...patch } } });
  };
  ...
  const addProject = () => {
    const p = makeProject({
      settings: { ...DEFAULT_SETTINGS, projectName: `New project ${data.projects.length + 1}`, startDate: toKey(new Date()) },
    });
  ...
  const deleteProject = (id) => {
    ...
    persist({
      ...data,
      projects: remaining,
      activeProjectId: data.activeProjectId === id ? remaining[0].id : data.activeProjectId,
    });
  };
...
const fmtDateLong = (k) => fromKey(k).toLocaleDateString(undefined, { month: "short", day: "numeric", year: "numeric" });

function TopBar({ view, setView, onOpenSetup, projectName, projectIcon, startDate, endDate, cloudEnabled, session, onSignOut }) {
  return (
      ...
      <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between gap-4">
        <div className="flex items-center gap-3 min-w-0">
          <div className="w-8 h-8 rounded-xl bg-[#1E2A33] flex items-center justify-center shrink-0">
            <RenderIcon name={projectIcon} size={16} className="text-[#F4F5F7]" />
          </div>
          <div className="min-w-0">
            <h1 className="font-sans font-extrabold uppercase tracking-tight text-lg leading-none truncate">{projectName}</h1>
            <p className="text-[11px] uppercase tracking-widest text-[#1E2A33]/50 font-mono mt-0.5">
              {startDate ? `${fmtDateLong(startDate)} → ${endDate ? fmtDateLong(endDate) : "ongoing"}` : "Route tracker & forecast"}
            </p>
          </div>
        </div>
...
function SetupModal({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-[2px] flex items-center justify-center p-4" onMouseDown={onBackdropClick}>
      <div style={{ backgroundColor: "#ffffff" }} className="w-full max-w-lg rounded-2xl shadow-xl border border-[#1E2A33]/10 max-h-[90vh] flex flex-col">
        <div style={{ backgroundColor: "#ffffff" }} className="flex items-center justify-between px-5 py-4 border-b border-[#1E2A33]/10 shrink-0 rounded-t-xl">
          <h2 className="font-sans font-extrabold uppercase tracking-tight text-sm">Setup</h2>
          <button onClick={onClose} className={`${btnBase} text-[#1E2A33]/50 hover:text-[#1E2A33]`}>
            <X size={18} />
          </button>
        </div>

        <div style={{ backgroundColor: "#ffffff" }} className="flex border-b border-[#1E2A33]/10 shrink-0">
          {[
            ...
            return (
              <button
                ...
                className={`${btnBase} flex-1 text-[10px] font-mono uppercase tracking-widest px-3 py-2.5 border-b-2 ${
                  active ? "" : "border-transparent text-[#1E2A33]/50 hover:text-[#1E2A33] hover:bg-[#1E2A33]/5"
                }`}
              >
                {t.label}
              </button>
        ...
        <div style={{ backgroundColor: "#ffffff" }} className="p-5 overflow-y-auto rounded-b-xl">
          {tab === "course" && <CourseTab settings={settings} onSave={onSaveCourse} />}
          {tab === "slots" && (
            <EditableList
              items={slots}
              onChange={onUpdateSlots}
              noun="slot"
              warningNote={(label) => `Remove "${label}"? Its logged time stays stored but won't appear in the log or analytics anymore.`}
            />
          )}
          {tab === "categories" && (
            <EditableList
              ...
              noun="category"
              warningNote={(label) => `Remove "${label}"? Entries already logged under it stay stored but will show as removed.`}
            />
          )}
          {tab === "projects" && (
            <ProjectsTab
...
function ProjectsTab({ projects, activeProjectId, onSwitch, onAdd, onDelete }) {
  ...
  return (
    <div className="space-y-2 font-mono text-sm">
      <p className="text-[10px] uppercase tracking-widest text-[#1E2A33]/50 mb-1">Switch between separate courses, each with its own slots, categories and log.</p>
      {projects.map((p) => {
        const active = p.id === activeProjectId;
        return (
          <div key={p.id} className={`border rounded-xl p-2.5 flex items-center gap-2.5 ${active ? "border-[#1E2A33]" : "border-[#1E2A33]/15"}`}>
            <span
              className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0"
              style={{ backgroundColor: active ? "#1E2A33" : "#1E2A330D", color: active ? "#fff" : "#1E2A33" }}
            >
              <RenderIcon name={p.settings.projectIcon} size={16} />
            </span>
            <div className="flex-1 min-w-0">
              <div className="text-xs font-bold truncate">{p.settings.projectName || "Untitled project"}</div>
              <div className="text-[10px] text-[#1E2A33]/40 truncate">
                {p.settings.startDate ? fmtDateLong(p.settings.startDate) : "No start date"}
                {p.settings.endDate ? ` → ${fmtDateLong(p.settings.endDate)}` : ""}
              </div>
            </div>
            {confirmDeleteId === p.id ? (
              ...
              <div className="flex items-center gap-1 shrink-0">
                ...
                {active && <span className="text-[9px] uppercase tracking-widest text-[#1E2A33]/40 px-1">Active</span>}
                <button
...
function CourseTab({ settings, onSave }) {
  const [projectName, setProjectName] = useState(settings.projectName ?? "Study Manifest");
  const [projectIcon, setProjectIcon] = useState(settings.projectIcon ?? "Train");
  ...
  const [startDate, setStartDate] = useState(settings.startDate || toKey(new Date()));
  const [endDate, setEndDate] = useState(settings.endDate || "");
  const [dailyGoals, setDailyGoals] = useState(settings.dailyGoals || DEFAULT_SETTINGS.dailyGoals);
  const [iconPickerOpen, setIconPickerOpen] = useState(false);

  useEffect(() => {
    const t = setTimeout(
      () => onSave({ projectName, projectIcon, totalLessons, totalExams, startDate, endDate: endDate || null, dailyGoals }),
      300
    );
    return () => clearTimeout(t);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [projectName, projectIcon, totalLessons, totalExams, startDate, endDate, dailyGoals]);

  const setGoal = (dayIdx, minutes) => setDailyGoals((g) => ({ ...g, [dayIdx]: Math.max(0, Number(minutes) || 0) }));

  return (
    <div className="space-y-5 font-mono text-sm">
      <div>
        <span className="block text-[10px] uppercase tracking-widest text-[#1E2A33]/50 mb-1">Project</span>
        <div className="flex items-center gap-2">
          <div className="relative">
            ...
            {iconPickerOpen && (
              <div className="absolute z-30 top-12 left-0 bg-white border border-[#1E2A33]/15 rounded-xl shadow-lg p-2.5 w-56">
                <p className="text-[9px] uppercase tracking-widest text-[#1E2A33]/40 mb-1.5">Project icon</p>
                  ...
                  {ICON_LIBRARY.map((opt) => (
                    <button
                      ...
                      className={`${btnBase} p-1.5 rounded-md hover:bg-[#1E2A33]/10 flex items-center justify-center ${
                        projectIcon === opt.name ? "bg-[#1E2A33]/10 ring-1 ring-[#1E2A33]/30" : ""
                      }`}
                    >
      ...
      <Field label="Course end date (optional)">
        <input
          ...
          className="w-full border border-[#1E2A33]/20 rounded-xl px-3 py-2"
        />
        <p className="text-[9px] text-[#1E2A33]/40 mt-1">Once set, days after this date won't count as "empty days" in Analytics.</p>
      </Field>
      ...
      <div>
        ...
        <div className="grid grid-cols-7 gap-1.5">
          {WEEKDAY_ORDER.map((idx) => (
            <label key={idx} className="flex flex-col items-center gap-1">
              <span className="text-[9px] uppercase tracking-widest text-[#1E2A33]/40">{WEEKDAY_LABELS[idx]}</span>
              <input
                type="number"
                min={0}
        ...
        <p className="text-[9px] text-[#1E2A33]/40 mt-1.5">Minutes per day. Shown as an hours goal on each day card in the Log tab.</p>
      </div>
    </div>
  );
}
...
function Field({ label, children }) {
  return (
    <label className="block">
      <span className="block text-[10px] uppercase tracking-widest text-[#1E2A33]/50 mb-1">{label}</span>
      {children}
    </label>
  );
}
...
function EditableList({ items, onChange, noun, warningNote }) {
  ...
  const addItem = () =>
    onChange([
      ...items,
      { id: makeId(noun), label: `New ${noun}`, iconName: "Star", color: PALETTE[items.length % PALETTE.length] },
    ]);
  const removeItem = (id) => {
  ...
  return (
    <div className="space-y-2">
      {items.map((item) => (
        <div key={item.id} className="border border-[#1E2A33]/15 rounded-xl p-2 bg-white">
          {confirmDeleteId === item.id ? (
            ...
            <div className="flex items-center gap-2">
              <div className="relative">
                ...
                {openPickerId === item.id && (
                  <div className="absolute z-30 top-10 left-0 bg-white border border-[#1E2A33]/15 rounded-xl shadow-lg p-2.5 w-56">
                    <p className="text-[9px] uppercase tracking-widest text-[#1E2A33]/40 mb-1.5">Icon</p>
                    <div className="grid grid-cols-6 gap-1 mb-3">
                      {ICON_LIBRARY.map((opt) => (
                        <button
                          key={opt.name}
                          onClick={() => updateItem(item.id, { iconName: opt.name })}
                          className={`${btnBase} p-1.5 rounded-md hover:bg-[#1E2A33]/10 flex items-center justify-center ${
                            item.iconName === opt.name ? "bg-[#1E2A33]/10 ring-1 ring-[#1E2A33]/30" : ""
                          }`}
                        >
                          <RenderIcon name={opt.name} size={14} />
                        </button>
                    ...
                    <p className="text-[9px] uppercase tracking-widest text-[#1E2A33]/40 mb-1.5">Color</p>
                    <div className="flex flex-wrap gap-1.5">
                      {PALETTE.map((c) => (
                        <button
                          ...
                          style={{
                            backgroundColor: c,
                            outline: item.color === c ? "2px solid #1E2A33" : "none",
                            outlineOffset: "1px",
                          }}
              ...
              <button
                ...
                className={`${btnBase} p-1.5 text-[#1E2A33]/40 hover:text-[#C1595B] disabled:opacity-20 disabled:cursor-not-allowed`}
                title={items.length <= 1 ? "At least one is required" : "Remove"}
              >
                <Trash2 size={14} />
              </button>
            </div>
...
function rangeLabel(cursor, granularity) {
  ...
  if (granularity === "90days") {
    ...
    return `${s.toLocaleDateString(undefined, { month: "short", day: "numeric" })} – ${cursor.toLocaleDateString(undefined, {
      month: "short",
      day: "numeric",
      year: "numeric",
    })}`;
...
function LogView({ data, granularity, setGranularity, cursor, setCursor, onNavigateDay, onEditDay }) {
  const { slots, categories, days, settings } = data;
  const todayKey = toKey(new Date());

  const headerStats = useMemo(() => {
    if (granularity === "week") return rangeStats(weekDates(cursor), days, slots, settings);
    if (granularity === "month") return rangeStats(monthDates(cursor), days, slots, settings);
    return null;
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
        <SegmentedControl items={GRANULARITIES} activeId={granularity} onChange={setGranularity} />
      ...
      <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1 mb-4">
        <h2 className="font-sans font-extrabold uppercase tracking-tight text-base">{rangeLabel(cursor, granularity)}</h2>
        {headerStats && (
      ...
      {granularity === "month" && (
        <MonthGrid cursor={cursor} days={days} slots={slots} categories={categories} settings={settings} todayKey={todayKey} onNavigateDay={onNavigateDay} onEditDay={onEditDay} />
      )}
      {granularity === "week" && (
        <FullCardGrid dates={weekDates(cursor)} days={days} slots={slots} categories={categories} settings={settings} todayKey={todayKey} onNavigateDay={onNavigateDay} onEditDay={onEditDay} />
      )}
      {granularity === "day" && (
        <FullCardGrid dates={[cursor]} days={days} slots={slots} categories={categories} settings={settings} todayKey={todayKey} onNavigateDay={onNavigateDay} onEditDay={onEditDay} big />
      )}
      {granularity === "90days" && (
        <Heatmap start={addDays(cursor, -89)} end={cursor} days={days} slots={slots} categories={categories} todayKey={todayKey} onSelectDay={onNavigateDay} showMonths />
      )}
      {granularity === "year" && (
        <Heatmap
      ...
      <div className="mt-4 flex flex-wrap gap-3 text-[10px] font-mono uppercase tracking-wide text-[#1E2A33]/60">
        {slots.map((s) => (
          <span key={s.id} className="flex items-center gap-1.5">
            <RenderIcon name={s.iconName} size={11} style={{ color: s.color }} />
            {s.label}
          </span>
        ))}
      </div>
...
function MonthGrid({ cursor, days, slots, categories, settings, todayKey, onNavigateDay, onEditDay }) {
  const year = cursor.getFullYear();
  const month = cursor.getMonth();
  ...
  return (
    <div>
      <div className="grid gap-2 mb-2 text-[10px] font-mono uppercase tracking-widest text-[#1E2A33]/50 text-center" style={gridCols}>
        <div />
      ...
      <div className="space-y-2">
        {weekRows.map((row, ri) => {
          const { total: wTotal, goal: wGoal } = rangeStats(row.filter(Boolean), days, slots, settings);
          return (
...
function WeekSummaryCell({ total, goal }) {
  ...
  return (
    <div className="rounded-2xl bg-[#1E2A33]/[0.04] border border-[#1E2A33]/10 border-dashed h-28 flex flex-col items-center justify-center px-1 text-center">
      <span className="text-[8px] font-mono uppercase tracking-widest text-[#1E2A33]/35 mb-1">Week</span>
      <span className="text-xs font-mono font-bold" style={met ? { color: GOAL_MET_COLOR } : undefined}>
        {total > 0 ? fmtHours(total) : "—"}
      </span>
      {goal > 0 && <span className="text-[8px] font-mono text-[#1E2A33]/35 mt-0.5">of {fmtHours(goal)}</span>}
    </div>
  );
}

function CompactDayCell({ date, entry, slots, categories, goal, isToday, isFuture, onNavigate, onEdit }) {
    ...
    >
      <div className="flex items-start justify-between">
        ...
        <div className="flex items-center gap-1">
          {entry?.exam && (
            <span title="Exam passed" className="flex items-center justify-center w-4 h-4 rounded-full" style={{ backgroundColor: EXAM_COLOR }}>
      ...
      <div className="flex flex-wrap gap-x-1.5 gap-y-0.5">
        {slots.map((s) =>
          bySlot[s.id] > 0 ? (
            <span key={s.id} className="flex items-center gap-0.5 text-[8px] font-mono font-bold" style={{ color: s.color }}>
              <span className="w-1 h-1 rounded-full" style={{ backgroundColor: s.color }} />
              {bySlot[s.id]}
            </span>
          ) : null
        )}
        {total === 0 && <span className="text-[8px] font-mono text-[#1E2A33]/25">—</span>}
      </div>

      <div className="flex items-center justify-between text-[10px] font-mono text-[#1E2A33]/70">
        <span style={metGoal ? { color: GOAL_MET_COLOR, fontWeight: 700 } : undefined}>
          {total > 0 ? fmtHours(total) : ""}
          {goal > 0 && <span className="text-[#1E2A33]/30">/{fmtHours(goal)}</span>}
        </span>
        {entry?.lessons > 0 && <span title="Lessons studied today">{entry.lessons}L</span>}
      </div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-2.5">
      {slots.map((slot) => {
        ...
        const slotMinutes = entries.reduce((a, e) => a + (Number(e.minutes) || 0), 0);
        return (
          <div key={slot.id}>
            <div className="flex items-center gap-1.5 mb-1">
              <span className="text-[9px] font-mono font-bold" style={{ color: slot.color }}>
                {fmtHours(slotMinutes)}
              </span>
              <RenderIcon name={slot.iconName} size={10} style={{ color: slot.color }} />
              <span className="text-[9px] uppercase tracking-widest font-mono font-bold" style={{ color: slot.color }}>
                {slot.label}
              </span>
            </div>
            <div className="space-y-1">
              {entries.map((e) => {
                ...
                return (
                  <div key={e.id} className="pl-3 border-l-2" style={{ borderColor: `${slot.color}30` }}>
                    ...
                    {e.comment && <div className="text-[10px] font-mono text-[#1E2A33]/50 italic mt-0.5 whitespace-pre-wrap">{e.comment}</div>}
                  </div>
                );
              })}
            </div>
...
function FullCardGrid({ dates, days, slots, categories, settings, todayKey, onNavigateDay, onEditDay, big }) {
  return (
    <div className={big ? "max-w-md" : "grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3"}>
      {dates.map((date) => (
        <FullDayCard
          key={toKey(date)}
          date={date}
...
function FullDayCard({ date, entry, slots, categories, goal, isToday, onNavigate, onEdit, big }) {
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        <div>
          <div className="font-mono text-sm font-bold">{date.toLocaleDateString(undefined, { weekday: "short", day: "numeric" })}</div>
        ...
        <div className="flex items-center gap-1.5">
          {entry?.exam && (
            <span className="flex items-center gap-1 text-[9px] uppercase tracking-wide font-mono text-white px-1.5 py-0.5 rounded-full" style={{ backgroundColor: EXAM_COLOR }}>
              <Award size={10} /> Exam
            </span>
          )}
          {entry?.lessons > 0 && (
            <span title="Lessons studied today" className="text-[9px] uppercase tracking-wide font-mono bg-[#1E2A33]/10 px-1.5 py-0.5 rounded-full">
              {entry.lessons}L
            </span>
          )}
        </div>
      ...
      <div className="flex items-baseline gap-1.5">
        <span className="text-lg font-mono font-extrabold" style={metGoal ? { color: GOAL_MET_COLOR } : undefined}>
          {total > 0 ? fmtHours(total) : "—"}
        </span>
        {goal > 0 && <span className="text-[10px] font-mono text-[#1E2A33]/35">goal {fmtHours(goal)}</span>}
      </div>

      {total === 0 ? (
        <p className="text-[10px] font-mono text-[#1E2A33]/35">No study logged — tap to add</p>
      ) : (
        <EntriesReadout slots={slots} categories={categories} cells={entry?.cells || {}} />
      )}
    </div>
  );
}
...
function Heatmap({ start, end, days, slots, categories, todayKey, onSelectDay, showMonths }) {
      ...
      <div className="flex min-w-max">
        {weeks.map((week, wi) => {
          ...
          if (showMonths && firstReal) {
            ...
            if (m !== lastMonth) {
              monthTag = firstReal.toLocaleDateString(undefined, { month: "short" });
              newMonth = lastMonth !== null;
              lastMonth = m;
            }
          }
          return (
            <div key={wi} className={`flex flex-col gap-1.5 ${newMonth ? "ml-6" : "ml-2"}`}>
              {showMonths && (
                <div className="h-3 text-[8px] font-mono text-[#1E2A33]/40 whitespace-nowrap">{monthTag}</div>
              )}
              {week.map((date, di) => {
                ...
                return (
                  <button
                    ...
                    style={{ backgroundColor: HEAT_SCALE[level], outline: isToday ? `2px solid ${ACCENT}` : "none", outlineOffset: "1px" }}
                    className={`${btnBase} w-10 h-10 rounded-lg hover:scale-105 flex flex-col items-center justify-center shrink-0`}
                  >
                    <span className={`text-[8px] font-mono leading-none ${darkText ? "text-[#1E2A33]/40" : "text-white/70"}`}>{date.getDate()}</span>
                    {total > 0 && (
                      <span className={`text-[9px] font-mono font-bold leading-none mt-0.5 ${darkText ? "text-[#1E2A33]/80" : "text-white"}`}>
                        {fmtHours(total)}
                      </span>
                    )}
                  </button>
...
function DayEditor({ dateKey, dayEntry, slots, categories, onClose, onChange }) {
  ...
  const addEntry = (slotId) => {
    ...
    const newEntry = { id: makeId("entry"), category: categories[0]?.id, minutes: 15, comment: "" };
    onChange({ cells: { ...cells, [slotId]: [...arr, newEntry] } });
  };
  const updateEntry = (slotId, entryId, patch) => {
    const arr = (cells[slotId] || []).map((e) => (e.id === entryId ? { ...e, ...patch } : e));
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-[2px] flex items-end sm:items-center justify-center p-0 sm:p-4" onMouseDown={onBackdropClick}>
      <div
      ...
      >
        <div style={{ backgroundColor: "#F4F5F7" }} className="flex items-center justify-between px-5 py-4 border-b border-[#1E2A33]/10 shrink-0">
          <div>
            <h2 className="font-sans font-extrabold uppercase tracking-tight text-sm">
              {d.toLocaleDateString(undefined, { weekday: "long", month: "long", day: "numeric" })}
            </h2>
            <p className="text-[10px] font-mono uppercase tracking-widest text-[#1E2A33]/50">{total} minutes logged · {fmtHours(total)}</p>
          </div>
          <button onClick={onClose} className={`${btnBase} text-[#1E2A33]/50 hover:text-[#1E2A33]`}>
            <X size={20} />
          </button>
        ...
        <div className="p-5 space-y-5 overflow-y-auto flex-1">
          {slots.map((slot) => {
            const entries = cells[slot.id] || [];
            const slotTotal = entries.reduce((a, e) => a + (Number(e.minutes) || 0), 0);
            return (
              <div key={slot.id} className={CARD}>
                ...
                >
                  <div className="flex items-center gap-2">
                    <RenderIcon name={slot.iconName} size={14} style={{ color: slot.color }} />
                    <span className="font-mono text-xs uppercase tracking-wide font-bold">{slot.label}</span>
                  </div>
                  <span className="font-mono text-xs text-[#1E2A33]/50">{slotTotal}m</span>
                </div>

                <div className="p-3 space-y-2">
                  {entries.length === 0 && <p className="text-xs font-mono text-[#1E2A33]/40 px-1">No study logged for this slot.</p>}
                  {entries.map((entry) => {
                    const options = categories.some((c) => c.id === entry.category)
                      ? categories
                      : [{ id: entry.category, label: `(removed) ${entry.category}` }, ...categories];
                    return (
                      <div key={entry.id} className="border border-[#1E2A33]/10 rounded-xl p-2 space-y-1.5">
                        <div className="flex items-center gap-2">
                          <select
                            value={entry.category}
                            onChange={(e) => updateEntry(slot.id, entry.id, { category: e.target.value })}
                          ...
                          <input
                            ...
                            min={0}
                            value={entry.minutes}
                            onChange={(e) => updateEntry(slot.id, entry.id, { minutes: Number(e.target.value) })}
                            className="w-20 border border-[#1E2A33]/20 rounded-xl px-2 py-1.5 text-xs font-mono"
                          />
                          <span className="text-[10px] font-mono text-[#1E2A33]/40">min</span>
                          <button
                        ...
                        <div className="flex items-start gap-1.5">
                          <MessageSquare size={12} className="text-[#1E2A33]/30 shrink-0 mt-1.5" />
                          <textarea
                            value={entry.comment || ""}
                            onChange={(e) => updateEntry(slot.id, entry.id, { comment: e.target.value })}
          ...
          <div className={`${CARD} p-4 flex items-center justify-between gap-4 flex-wrap`}>
            ...
            <label className="flex items-center gap-2 text-xs font-mono uppercase tracking-wide cursor-pointer">
              ...
              <span className="flex items-center gap-1">
                <Award size={13} style={{ color: EXAM_COLOR }} /> Exam passed today
              </span>
            </label>
          </div>
        </div>
...
function AnalyticsView({ data }) {
  ...
  const { rangeStart, rangeEnd } = useMemo(() => {
    ...
    if (preset === "custom") return { rangeStart: fromKey(customStart), rangeEnd: fromKey(customEnd) };
    if (preset === "all") {
      const first = dayKeysSorted[0] ? fromKey(dayKeysSorted[0]) : settings.startDate ? fromKey(settings.startDate) : today;
  ...
  const overall = useMemo(() => {
    ...
    const start = settings.startDate ? fromKey(settings.startDate) : dayKeysSorted[0] ? fromKey(dayKeysSorted[0]) : new Date();
    const today = new Date();
    const cutoff = settings.endDate && fromKey(settings.endDate) < today ? fromKey(settings.endDate) : today;
    ...
    const avgMinutesPerLesson = lessonsDone > 0 ? totalMinutes / lessonsDone : null;
    const avgLessonsPerActiveDay = activeDays > 0 ? lessonsDone / activeDays : null;
    ...
    if (avgMinutesPerLesson && lessonsRemaining > 0) estRemainingMinutes = avgMinutesPerLesson * lessonsRemaining;
    if (avgLessonsPerActiveDay && lessonsRemaining > 0) {
      const estActiveDaysNeeded = lessonsRemaining / avgLessonsPerActiveDay;
      estRemainingCalendarDays = activeRatio > 0 ? Math.ceil(estActiveDaysNeeded / activeRatio) : Math.ceil(estActiveDaysNeeded);
    ...
    const avgHoursPerDay = daysSinceStart > 0 ? totalMinutes / 60 / daysSinceStart : null;
    const avgHoursPerLesson = avgMinutesPerLesson != null ? avgMinutesPerLesson / 60 : null;
    const avgLessonsPerDay = daysSinceStart > 0 ? lessonsDone / daysSinceStart : null;
  ...
  const dailyTotals = useMemo(
    () =>
      rangedKeys.map((k) => {
        ...
        const row = { date: fmtShort(k), total: toHours(total), lessons: Number(entry.lessons) || 0 };
        if (dailyMode === "slot") {
          slots.forEach((s) => (row[s.id] = toHours(bySlot[s.id])));
        } else if (dailyMode === "category") {
          categories.forEach((c) => (row[c.id] = toHours(byCategory[c.id] || 0)));
        }
        return row;
      }),
    [rangedKeys, days, slots, categories, dailyMode]
  );
  const dailySeries = dailyMode === "slot" ? slots : dailyMode === "category" ? categories : [];
  ...
  const slotTotals = useMemo(() => {
    ...
    return slots.map((s) => ({ id: s.id, name: s.label, value: toHours(sums[s.id]), color: s.color }));
  ...
  const effectivenessBySlot = useMemo(() => {
    ...
    return slots.map((s) => ({
      ...
      slotId: s.id,
      color: s.color,
      lessonsPerHour: hourSum[s.id] > 0 ? Number((lessonMinutes[s.id] / hourSum[s.id]).toFixed(2)) : 0,
    }));
  }, [rangedKeys, days, slots]);
  const visibleEffData = effectivenessBySlot.filter((s) => !effToggle.hidden.has(s.slotId));
  ...
  const effectivenessTrend = useMemo(() => {
    return weeklyBuckets.map(([wk, keys]) => {
      ...
      slots.forEach((s) => {
        row[s.id] = hourSum[s.id] > 0 ? Number((lessonMinutes[s.id] / hourSum[s.id]).toFixed(2)) : null;
      });
      return row;
    });
  ...
  const weeklyOverall = useMemo(() => {
    return weeklyBuckets.map(([wk, keys]) => {
      ...
      return { week: fmtShort(wk), lessonsPerHour: hours > 0 ? Number((lessons / hours).toFixed(2)) : 0, minutes };
    });
  }, [weeklyBuckets, days, slots]);
  ...
  const weekLabelsList = useMemo(() => weeklyBuckets.map(([wk]) => fmtShort(wk)), [weeklyBuckets]);
  ...
  return (
    <div className="space-y-8">
      <RangePicker preset={preset} setPreset={setPreset} customStart={customStart} setCustomStart={setCustomStart} customEnd={customEnd} setCustomEnd={setCustomEnd} />
        ...
        <ResponsiveContainer width="100%" height={260}>
          <ComposedChart data={dailyTotals}>
            ...
            <Tooltip
              ...
              formatter={(value, name) => (dailyMode === "lessons" ? [`${value}`, name] : [fmtHoursChart(value), name])}
            />
            {dailyMode === "hours" ? (
              <Area type="monotone" dataKey="total" stroke={ACCENT} fill={ACCENT} fillOpacity={0.25} name="Hours studied" />
            ) : dailyMode === "lessons" ? (
              <Area type="monotone" dataKey="lessons" stroke={GOAL_MET_COLOR} fill={GOAL_MET_COLOR} fillOpacity={0.25} name="Lessons" />
            ) : (
              ...
              [
                ...dailySeries
                  ...
                  .map((s) => (
                    <Area key={s.id} type="monotone" dataKey={s.id} stackId="a" stroke={s.color} fill={s.color} fillOpacity={0.55} name={s.label} />
                  )),
                <Line key="total-line" type="monotone" dataKey="total" stroke="#1E2A33" strokeWidth={1.5} strokeDasharray="5 3" dot={false} name="Total hours" />,
              ]
            )}
          </ComposedChart>
        </ResponsiveContainer>
        {(dailyMode === "slot" || dailyMode === "category") && (
          <ToggleChips items={dailySeries} hidden={dailyToggle.hidden} onToggle={dailyToggle.toggle} />
        )}
      </ChartCard>

      <ChartCard title="Days per exam" subtitle="Calendar days needed to reach each exam, all-time">
        {examsGapData.length === 0 ? (
          <p className="text-xs font-mono text-[#1E2A33]/40 py-10 text-center">
            No exams passed yet — this fills in as you mark exam days in the log.
          </p>
        ) : (
          <ResponsiveContainer width="100%" height={240}>
            <AreaChart data={examsGapData}>
              <CartesianGrid strokeDasharray="3 3" stroke="#1E2A3315" />
              <XAxis dataKey="exam" tick={{ fontSize: 10, fontFamily: "monospace" }} />
              <YAxis tick={{ fontSize: 10, fontFamily: "monospace" }} />
              <Tooltip
                contentStyle={{ fontSize: 12, fontFamily: "monospace" }}
                formatter={(value, _name, props) => [`${value} days`, `Passed ${props.payload.date}`]}
              />
              {overall.avgDaysPerExam != null && (
                <ReferenceLine
                  ...
                  stroke={ACCENT}
                  strokeDasharray="4 4"
                  label={{ value: `avg ${overall.avgDaysPerExam.toFixed(0)}d`, fontSize: 10, fill: ACCENT, position: "right" }}
                />
              )}
              <Area type="monotone" dataKey="days" stroke={ACCENT} fill={ACCENT} fillOpacity={0.28} strokeWidth={2} name="Days needed" dot={{ r: 4, fill: ACCENT, strokeWidth: 0 }} />
            </AreaChart>
          </ResponsiveContainer>
        )}
      </ChartCard>

      <ChartCard title="Weekly effectiveness" subtitle="Overall lessons/hour, aggregated per week">
        <ResponsiveContainer width="100%" height={260}>
          <LineChart data={weeklyOverall}>
            ...
            <Line type="monotone" dataKey="lessonsPerHour" stroke="#1E2A33" strokeWidth={2} name="Lessons / hour" dot={{ r: 3 }} />
          </LineChart>
        </ResponsiveContainer>
      </ChartCard>
      ...
      >
        <ResponsiveContainer width="100%" height={280}>
          <AreaChart data={weekdayData}>
            <CartesianGrid strokeDasharray="3 3" stroke="#1E2A3315" />
            <XAxis dataKey="weekday" tick={{ fontSize: 10, fontFamily: "monospace" }} />
            <YAxis tick={{ fontSize: 10, fontFamily: "monospace" }} tickFormatter={weekdayMode === "lessons" ? undefined : fmtAxisHours} />
            <Tooltip
              contentStyle={{ fontSize: 12, fontFamily: "monospace" }}
              formatter={(value, name) => (weekdayMode === "lessons" ? [`${value}`, name] : [fmtHoursChart(value), name])}
            />
            {weekdaySeries
              ...
              .map((s) =>
                weekdayMode === "slot" || weekdayMode === "category" ? (
                  ...
                  <Area key={s.id} type="monotone" dataKey={s.id} stroke={s.color} fill={s.color} fillOpacity={0.1} strokeWidth={2} name={s.label} dot={{ r: 2 }} />
                )
              )}
          </AreaChart>
        </ResponsiveContainer>
        <ToggleChips items={weekdaySeries} hidden={weekdayToggle.hidden} onToggle={weekdayToggle.toggle} />
      </ChartCard>

      <div className="grid md:grid-cols-2 gap-6">
        <ChartCard title="Time by slot type" subtitle="Share of total hours in this range — click a chip to toggle it out">
          <ResponsiveContainer width="100%" height={260}>
            <PieChart>
              <Pie data={visiblePieData} dataKey="value" nameKey="name" innerRadius={55} outerRadius={90} paddingAngle={2}>
                {visiblePieData.map((s, i) => (
                  <Cell key={i} fill={s.color} />
                ))}
              </Pie>
              <Tooltip contentStyle={{ fontSize: 12, fontFamily: "monospace" }} formatter={(value, name) => [fmtHoursChart(value), name]} />
            </PieChart>
          </ResponsiveContainer>
          <ToggleChips items={slotTotals} hidden={pieToggle.hidden} onToggle={pieToggle.toggle} />
        </ChartCard>

        <ChartCard title="Effectiveness by slot" subtitle="Estimated lessons completed per hour">
          <ResponsiveContainer width="100%" height={260}>
            <BarChart data={visibleEffData}>
              <CartesianGrid strokeDasharray="3 3" stroke="#1E2A3315" />
              <XAxis dataKey="name" tick={{ fontSize: 9, fontFamily: "monospace" }} interval={0} angle={-15} textAnchor="end" height={50} />
              <YAxis tick={{ fontSize: 10, fontFamily: "monospace" }} />
              <Tooltip contentStyle={{ fontSize: 12, fontFamily: "monospace" }} />
          ...
          <ToggleChips items={effectivenessBySlot.map((s) => ({ id: s.slotId, label: s.name, color: s.color }))} hidden={effToggle.hidden} onToggle={effToggle.toggle} />
        </ChartCard>
      </div>

      <ChartCard title="Effectiveness trend by slot" subtitle="Weekly lessons/hour for each study slot over time">
        <ResponsiveContainer width="100%" height={280}>
          <LineChart data={effectivenessTrend}>
            ...
            {slots
              ...
              .map((s) => (
                <Line key={s.id} type="monotone" dataKey={s.id} stroke={s.color} strokeWidth={2} name={s.label} connectNulls dot={{ r: 2 }} />
              ))}
          </LineChart>
        </ResponsiveContainer>
        <ToggleChips items={slots} hidden={trendToggle.hidden} onToggle={trendToggle.toggle} />
      </ChartCard>
    </div>
  );
}

function RangePicker({ preset, setPreset, customStart, setCustomStart, customEnd, setCustomEnd }) {
  return (
    <div className="flex flex-wrap items-center gap-2">
      <SegmentedControl items={RANGE_PRESETS} activeId={preset} onChange={setPreset} />
      {preset === "custom" && (
...
function OverviewStats({ overall }) {
  ...
  const lessonPct = overall.totalLessons > 0 ? Math.min(100, Math.round((overall.lessonsDone / overall.totalLessons) * 100)) : 0;
  const examPct = overall.totalExams > 0 ? Math.min(100, Math.round((overall.examsDone / overall.totalExams) * 100)) : 0;

  const stats = [
    { label: "Hours studied", value: hours, icon: Clock },
    { label: "Days since start", value: overall.daysSinceStart, icon: CalendarDays },
    { label: "Empty days", value: overall.emptyDays, icon: AlertCircle },
    { label: "Lessons done", value: `${overall.lessonsDone}/${overall.totalLessons}`, sub: `${lessonPct}%`, icon: TrendingUp },
    { label: "Exams passed", value: `${overall.examsDone}/${overall.totalExams}`, sub: `${examPct}%`, icon: Award },
  ];

  return (
    <div>
      <h3 className="font-sans font-extrabold uppercase tracking-tight text-sm mb-1 text-[#1E2A33]">Stats</h3>
      <p className="text-[11px] font-mono text-[#1E2A33]/40 mb-3 uppercase tracking-widest">Overview, all-time</p>
        ...
        {stats.map((s, i) => {
          ...
          return (
            <div
              ...
              className={
                dark
                  ? "bg-[#1E2A33] text-white rounded-2xl p-4 shadow-[0_4px_16px_rgba(20,26,33,0.18)]"
                  : `${CARD} p-4`
              }
            >
              <div className="flex items-center justify-between mb-3">
                <span className={`text-[9px] font-mono uppercase tracking-widest ${dark ? "text-white/55" : "text-[#1E2A33]/50"}`}>
                  {s.label}
                </span>
                <span className={`flex items-center justify-center w-6 h-6 rounded-full ${dark ? "bg-white/10" : "bg-[#1E2A33]/5"}`}>
                  <Icon size={12} className={dark ? "text-white/85" : "text-[#1E2A33]/40"} />
                </span>
              </div>
              <div className="flex items-baseline gap-1.5">
                <span className="font-mono text-xl font-bold">{s.value}</span>
                {s.sub && <span className={`text-[10px] font-mono ${dark ? "text-white/40" : "text-[#1E2A33]/40"}`}>{s.sub}</span>}
              </div>
            </div>
          );
        })}
...
function AveragesStats({ overall }) {
  const items = [
    { label: "Avg hours / day", value: overall.avgHoursPerDay != null ? `${overall.avgHoursPerDay.toFixed(1)}h` : "—", icon: Clock },
    { label: "Avg hours / lesson", value: overall.avgHoursPerLesson != null ? `${overall.avgHoursPerLesson.toFixed(2)}h` : "—", icon: BookOpen },
    { label: "Avg lessons / day", value: overall.avgLessonsPerDay != null ? overall.avgLessonsPerDay.toFixed(2) : "—", icon: TrendingUp },
    { label: "Avg days / exam", value: overall.avgDaysPerExam != null ? Math.round(overall.avgDaysPerExam) : "—", icon: Award },
  ];

  return (
    <div>
      <h3 className="font-sans font-extrabold uppercase tracking-tight text-sm mb-1 text-[#1E2A33]">Averages</h3>
      <p className="text-[11px] font-mono text-[#1E2A33]/40 mb-3 uppercase tracking-widest">All-time pace</p>
        ...
        {items.map((it) => {
          ...
          return (
            <div key={it.label} className={`${CARD} p-4`}>
              <div className="flex items-center justify-between mb-3">
                <span className="text-[9px] font-mono uppercase tracking-widest text-[#1E2A33]/50">{it.label}</span>
...
function PrognosisCard({ overall }) {
  const hasEnough = overall.avgMinutesPerLesson && overall.avgLessonsPerActiveDay;

  return (
    <div className="bg-[#1E2A33] text-[#F4F5F7] rounded-2xl p-5 shadow-sm">
      <h3 className="font-sans font-extrabold uppercase tracking-tight text-sm mb-1">Forecast</h3>
      <p className="text-[11px] font-mono text-[#F4F5F7]/50 mb-4 uppercase tracking-widest">Based on pace so far, all-time</p>

      {!hasEnough ? (
        <p className="text-sm font-mono text-[#F4F5F7]/70">Log a few more study days with lessons completed to unlock a forecast.</p>
      ) : overall.lessonsRemaining === 0 ? (
        <p className="text-sm font-mono">Course complete — all {overall.totalLessons} lessons logged. 🎉</p>
      ) : (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <ForecastStat label="Lessons remaining" value={overall.lessonsRemaining} />
          <ForecastStat label="Est. time remaining" value={overall.estRemainingMinutes ? `${(overall.estRemainingMinutes / 60).toFixed(1)}h` : "—"} />
          <ForecastStat
          ...
          <ForecastStat
            ...
            value={overall.estFinishDate ? overall.estFinishDate.toLocaleDateString(undefined, { month: "short", day: "numeric", year: "numeric" }) : "—"}
          />
        </div>
      )}
    </div>
...
function ForecastStat({ label, value }) {
  return (
    <div>
      <div className="text-[10px] font-mono uppercase tracking-widest text-[#F4F5F7]/50 mb-1">{label}</div>
  ...
  return (
    <div className={`${CARD} p-4`}>
      <div className="flex items-start justify-between gap-3 mb-3 flex-wrap">
        <div>
          <h3 className="font-sans font-extrabold uppercase tracking-tight text-sm">{title}</h3>
          {subtitle && <p className="text-[10px] font-mono uppercase tracking-widest text-[#1E2A33]/40">{subtitle}</p>}
        </div>
        {action}
      </div>
      {children}
